...
def test_dirty_overlap_blocks_without_moving_head_branch_or_index(
    ...
    stale_gate = blocked["artifacts"]["review_gate"]
    resumed = app.workflow_resume(
        run["run_id"],
        reruns={"implement.code": "revalidate scoped dirty overlap"},
    )
    assert "review_gate" not in resumed["artifacts"]
    assert app.workflow_status(run["run_id"])["status"] == "pending"

    state_path = project / ".ai-workflow" / "runs" / run["run_id"] / "state.yaml"
    legacy_state = yaml.safe_load(state_path.read_text(encoding="utf-8"))
    legacy_state["artifacts"]["review_gate"] = stale_gate
    state_path.write_text(
        yaml.safe_dump(legacy_state, sort_keys=True), encoding="utf-8"
    )

    recovered = app.workflow_status(run["run_id"])
    assert "review_gate" not in recovered["artifacts"]
    app.workflow_begin(run["run_id"], "implement")
    persisted = yaml.safe_load(state_path.read_text(encoding="utf-8"))
    assert "review_gate" not in persisted["artifacts"]


def test_claimed_dirty_path_recovers_checkpoint_without_new_code_change(
    tmp_path: Path, project_template: ProjectTemplate
) -> None:
    project = project_template.copy_to(tmp_path / "claimed-dirty-repo")
    app = CliDriver(project)
    agent = FakeAgent(project)
    run = app.workflow_init(profile="full")
    for phase in ("spec", "plan"):
    ...
    app.workflow_finalize(run["run_id"], implement["attempt_id"])
    first_gate = app.workflow_review(run["run_id"])
    status, payload = app.workflow_review_accept_status(
        run["run_id"], first_gate["digest"]
    )

    assert status != 0
    assert payload["error"]["code"] == "checkpoint_scope_ambiguous"
    resumed = app.workflow_resume(
        run["run_id"], claim_paths=("src/app.py",)
    )
    claim = resumed["artifacts"]["_checkpoint_claim"]
    assert [item["path"] for item in claim["snapshots"]] == ["src/app.py"]
    assert claim["snapshots"][0]["digest"]

    second_gate = app.workflow_review(run["run_id"])
    status, payload = app.workflow_review_accept_status(
        run["run_id"], second_gate["digest"]
    )

    assert status == 0
    assert payload["data"]["decision"] == "human_review"
    transitioned = app.workflow_transition(run["run_id"])
    assert transitioned["current_phase"] == "verify"
    assert "_checkpoint_claim" not in transitioned["artifacts"]
    assert "_checkpoint_failure" not in transitioned["artifacts"]
    assert "src/app.py" in transitioned["artifacts"]["checkpoints"]["active"][
        "included_paths"
    ]


def test_implementation_rerun_discards_claim_from_previous_attempt(
    tmp_path: Path, project_template: ProjectTemplate
) -> None:
    project = project_template.copy_to(tmp_path / "discarded-claim-repo")
    app = CliDriver(project)
    agent = FakeAgent(project)
    run = app.workflow_init(profile="full")
    for phase in ("spec", "plan"):
    ...
    app.workflow_finalize(run["run_id"], implement["attempt_id"])
    gate = app.workflow_review(run["run_id"])
    status, _ = app.workflow_review_accept_status(run["run_id"], gate["digest"])
    assert status != 0
    resumed = app.workflow_resume(run["run_id"], claim_paths=("src/app.py",))
    assert "_checkpoint_claim" in resumed["artifacts"]

    app.workflow_block(run["run_id"], "implementation scope changed")
    rerun = app.workflow_resume(
        run["run_id"],
        reruns={"implement.code": "remove the previously claimed implementation"},
    )

    assert "_checkpoint_claim" not in rerun["artifacts"]
    assert "_checkpoint_failure" not in rerun["artifacts"]
    assert "implement" not in rerun["artifacts"]["current_attempts"]
